...
export default async function AdminDashboardPage(props: { searchParams: Promise<{ branchId?: string }> }) {
    const session = await auth();
    const isAdmin = session?.user?.role === 'ADMIN';
    const userBranchId = session?.user?.branchId;

    // Get search param for admin filtering
    const searchParams = await props.searchParams;
    let selectedBranchId = isAdmin ? searchParams.branchId : userBranchId;
    if (selectedBranchId === "ALL") selectedBranchId = undefined; // ALL means no filter

    // Filters for branch isolation
    ...
    const totalOrders = await prisma.order.count({ where: branchFilter });
    const totalProducts = await prisma.product.count();
    const totalUsers = await prisma.user.count({ where: branchFilter });
    ...
    const orderItems = await prisma.orderItem.findMany({
        where: {
            order: { ...branchFilter, status: { in: ['PAID', 'PROCESSING', 'COMPLETED'] } }
        },
