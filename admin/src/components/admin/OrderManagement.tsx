...
const formatOptions = (optionsStr: string | null) => {
    ...
    try {
        ...
        const parts: string[] = [];
        // Legacy
        if (options.sweetness) parts.push(`หวาน: ${options.sweetness}`);
        if (Array.isArray(options.addons) && options.addons.length > 0) parts.push(`+ ${options.addons.join(", ")}`);
        // Generic
        Object.entries(options).forEach(([k, v]) => {
            if (k !== 'sweetness' && k !== 'addons' && v && v !== 'none') {
                // Format key if needed, or just show Value
                // Showing Key: Value is safer
                parts.push(`${k}: ${v}`);
            }
        });
...
export function OrderManagement({ initialOrders }: Props) {
    ...
    const filteredOrders = initialOrders.filter(order => {
        ...
        // Create boundaries in local time
        const selectedEnd = new Date(selectedDate);
        selectedEnd.setHours(23, 59, 59, 999); // End of selected day (Local)

        // Adjust selected to ensure it covers the full day in local time when created from YYYY-MM-DD
        // If YYYY-MM-DD is parsed as UTC, we need to adjust
        // But better is to just compare dates ignoring time if possible, or use consistent timestamps.

        // Let's use simple string comparison on local date strings for reliability
        const orderDateStr = orderDate.toLocaleDateString('en-CA'); // YYYY-MM-DD local
        const selectedDateObj = new Date(selectedDate);

        // Check if order date is within [selectedDate - 15 days, selectedDate]
        // We can just compare timestamps of "start of day"

        const orderTime = new Date(orderDateStr).getTime();
        const selectedTime = new Date(selectedDate).getTime();
        const fifteenDaysTime = fifteenDaysAgo.getTime();

        // Allow if orderDate <= selectedDate AND orderDate >= fifteenDaysAgo
        // Using string comparison for "same day" logic is easier
        if (orderDateStr > selectedDate) return false;

        // 15 days ago calculation might be tricky with strings, so fallback to timestamp logic
        // Re-calculate borders
        const daysDiff = (selectedTime - orderTime) / (1000 * 60 * 60 * 24);

        if (daysDiff < 0 || daysDiff > 15) return false;
    ...
    const printOrderLabel = (order: ExtendedOrder) => {
        ...
        const itemsList = order.items.map(item => {
            ...
            return `<div style="display: flex; justify-content: space-between; padding: 4px 0; border-bottom: 1px dashed #ddd;">
                <div>
                    <span>${item.Product.name}</span>
                    ${options ? `<div style="font-size: 10px; color: #666; margin-left: 5px;">${options}</div>` : ''}
                </div>
        ...
        printWindow.document.write(`
            ...
            <body>
                <div class="label">
                    <div class="header">
                        <div style="font-size: 18px; font-weight: bold;">ODM VIDWA</div>
                                            ...
                                            <div className="flex flex-col gap-0.5 max-w-[200px]">
                                                {order.items.slice(0, 2).map((item, idx) => (
                                                    <div key={idx} className="flex flex-col gap-0.5">
                                                        ...
                                                        {item.options && (
                                                            <span className="text-[10px] text-gray-500 pl-2 truncate block">
